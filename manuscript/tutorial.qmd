...
# Introduction
Earth observations (EO) provide an effective means of monitoring the global distribution of greenhouse gases with high spatiotemporal resolution and are considered a valuable complement to ground-based (GB) observations by improving overall monitoring coverage, frequency and resolution [@yue2016space;@hu2024review].

However, integrating EO and GB datasets presents methodological challenges. EO data represent column-averaged dry-air mole fractions ($X_{CO_2}$ or $X_{CH_4}$), while GB instruments measure near-surface concentrations ($CO_2$ or $CH_4$). Systematic discrepancies arise from vertical gradients in atmospheric composition, which differ substantially between $CO_2$ (relatively well-mixed) and $CH_4$ (pronounced vertical gradients). A mapping is therefore required to calibrate EO measurements to GB-equivalent surface values, accounting for these physical differences while propagating uncertainty appropriately.

This work, we use a statistical approach to compute this mapping using a Bayesian hierarchical modeling framework for assimilating EO and GB data to generate unified $CO_2$ and $CH_4$ concentration datasets suitable for CMIP applications. The approach employs Gaussian Process components to capture spatial structure and hierarchical seasonality to account for latitudinal variations. We demonstrate the methodology using OBS4MIPS satellite products and NOAA/AGAGE ground-based measurements, generating composite datasets that extend GB coverage globally while maintaining explicit uncertainty quantification. Additionally, we provide an initial idea for a forecasting model to project concentrations into future years, enabling scenario development for climate projections.

The remainder of this paper is structured as follows. @sec-datasets introduces the GB and EO datasets used in the analysis. @sec-motivation recapture the motivationg and the challenge for the current analysis. @sec-method outlines the methodological framework, including the Bayesian calibration model for data integration and the forecasting approach for temporal extrapolation. @sec-results presents results for both $CO_2$ and $CH_4$, evaluating model performance and examining regional patterns. Finally, @sec-discussion discusses the findings and limitations.

# Datasets {#sec-datasets}
In the following section we introduce the ground-based (GB) and earth observation (EO) data set used throughout this working paper.
...
Distinct model specifications were developed for $CO_2$ and $CH_4$ to account for their unique physicochemical properties. These specifications are detailed in the following section.
...
$$
\mu_i = \underbrace{\beta_0 + \beta_1 t_i + \beta_2 t_i^2}_{\substack{\text{Global Trend}}} + \underbrace{\mathcal{S}_{g[i]}(t_i)}_{\substack{\text{Seasonality}}} + \underbrace{x_{i, \text{eo}}}_{\substack{\text{EO Calibration}}} + \underbrace{f_{\text{HSGP}}(\phi_i, \lambda_i)}_{\substack{\text{Spatial Residual}}}
$$
where $t_i$ represents the normalized time variable, and $x_{i, \text{eo}}$ is the scaled EO measurement. The term $\mathcal{S}_{g[i]}(t_i)$ denotes the seasonal cycle, where the harmonic coefficients are allowed to vary by the grouping factor $g[i]$ (defined by the interaction of latitude and hemispheric region (S for lat < 0, N else)).
...
```
value_gb ~
    1 + year_scaled + I(year_scaled**2) +
    (1 + season_sin + season_cos | lat : region) +
    value_eo_scaled +
    hsgp(lat_scaled, lon_scaled, m=5, c=1.5)
```
...
+ *Region Definition:* The variable `region` was constructed as a binary spatial indicator to distinguish between the Northern and Southern Hemispheres ($0$ for latitude $< 0^{\circ}$, $1$ otherwise).
...
+ *Global Trend*: The term `1 + year_scaled + I(year_scaled**2)` captures the long-term accumulation of $CO_2$ in the atmosphere. A quadratic polynomial is included to account for non-linearities in the accumulation rate, such as the accelerating growth observed in recent years.
+ *Seasonality*: The random effects term `(1 + season_sin + season_cos | lat : region)` models the seasonal cycle using harmonic basis functions. Unlike a fixed global seasonality, this hierarchical structure estimates specific seasonal amplitudes and phases for distinct groups defined by the interaction of latitude and hemispheric region `(lat : region)`. This allows the model to adapt to local transport patterns.
+ *EO-Calibration*: The term `value_eo_scaled` estimates a linear relationship between EO and GB and applies a uniform correction coefficient.
+ *Spatial Residuals*: The final term `hsgp(lat_scaled, lon_scaled, m=5, c=1.5)` captures the static spatial structure not explained by the other covariates. The HSGP approximation projects the computationally expensive Gaussian Process covariance matrix inversion onto a summation of basis functions (eigenfunctions of the Laplacian), thereby facilitating efficient scaling to large global datasets.

#### Model specification $CH_4$ {.unnumbered}
The model specification for $CH_4$ is analogous to that of $CO_2$, with one distinction: the fixed main effect of EO is substituted with the interaction term `value_eo_scaled : lat_scaled`. This interaction serves as a spatially adaptive calibration layer, permitting the slope of the relationship between EO and GB data to vary across latitudes.
...
value_gb ~
    1 + year_scaled + I(year_scaled**2) +
    value_eo_scaled +
    (1 + season_sin + season_cos + value_eo_scaled | lat) +
    hsgp(lat_scaled, lon_scaled, m=5, c=1.5)
"""
...
plotting.plot_global_hemisphere_coverage(
    obs_values[(obs_values.year > 2015)],
    df_partial_coverage_co2[df_partial_coverage_co2.year > 2015],
    gas="co2",
    unit="ppm",
    split_value=SPLIT_VALUE,
    figsize=(5,4)
...
if os.path.exists(cache_df_predicted_co2):
    ...
    df_predicted_co2 = run_prediction_model(
        df_partial_coverage_co2[df_partial_coverage_co2.year > 2017],
...
prophet_components_co2 = analysis.extract_prophet_components(
    ...
    df_partial_coverage_co2[df_partial_coverage_co2.year > 2017],
    future_time_range=(2017, 2030),
    split_value=SPLIT_VALUE
)

plotting.plot_prophet_components(
...
cache_idata_ch4 = "datasets/idata_ch4_6.nc"
#
#value_gb ~
#    1 + year_scaled + I(year_scaled**2) +
#    value_eo_scaled + value_eo_scaled:lat_scaled +
#    (1 + value_eo_scaled + season_sin + season_cos | lat) +
#    hsgp(lat_scaled, lon_scaled, m=15, c=1.5)

formula_ch4 = """
value_gb ~
    1 +
    hsgp(lat_scaled, lon_scaled, m=5, c=1) +
    (
        1 + value_eo_scaled +
        year_scaled + I(year_scaled**2) +
        season_sin + season_cos +
        season_sin2 + season_cos2 | hemisphere
    )
"""
df_collocated_ch4 = preprocessing.add_hemisphere(df_collocated_ch4, SPLIT_VALUE)
...
cache_df_gb_pred_ch4 = "datasets/df_gb_pred_ch4_5.csv"
...
cache_df_predicted_ch4 = "datasets/df_predicted_ch4_5.csv"
...
var_names_co2 = [
    ...
    "season_cos|lat",
    "value_eo_scaled|lat",
    "hsgp(lat_scaled, lon_scaled, m=5, c=1.5)_ell",
    "sigma"
    ]
```
...
var_names_ch4 = [
    "Intercept", "bs(year_scaled, df=4)","1|hemisphere",
    "value_eo_scaled|hemisphere", "season_sin|hemisphere",
    "season_cos|hemisphere", "season_sin2|hemisphere",
    "season_cos2|hemisphere","hsgp(lat_scaled, lon_scaled, m=20, c=2)_ell"
]
```
...
# Posterior Predictive Validation {#sec-validation-posterior-predictive}

Beyond MCMC convergence diagnostics, we validate the fitted models by comparing posterior predictions to observed data. While MCMC diagnostics ensure that the sampling algorithm has converged to the target posterior distribution, posterior predictive validation assesses whether the model itself adequately captures the data-generating process. This section presents comprehensive posterior predictive checks to assess model calibration, residual patterns, coverage properties, and quantitative performance metrics.

**Interpretation Guidelines:**
- **Posterior Predictive Checks (PPC)**: The observed data should fall within the distribution of simulated data. Systematic deviations suggest model misspecification.
- **Residual Analysis**: Residuals should be randomly distributed around zero with constant variance. Patterns or trends indicate systematic biases or heteroscedasticity.
- **Coverage Statistics**: Coverage percentages close to the nominal levels (50%, 80%, 95%) indicate well-calibrated uncertainty quantification. Over-coverage suggests conservative uncertainty estimates, while under-coverage indicates overconfident predictions.
- **Quantitative Metrics**: Lower RMSE and MAE values indicate better predictive accuracy, while R² values closer to 1.0 indicate better explanatory power.
...
Residual analysis examines the differences between observed and predicted values to identify systematic patterns, heteroscedasticity, or outliers that may indicate model misspecification.

```{python}
#| label: app-validation-residuals-co2

model_co2_pred = model_co2
model_co2_pred.predict(idata_co2, kind="response")

# Compute residuals using posterior mean predictions
y_obs = df_collocated_co2["value_gb"].values
y_pred_mean = idata_co2.posterior_predictive["value_gb"].mean(dim=("chain", "draw")).values
...
axs[0].scatter(y_pred_mean, residuals, alpha=0.5, s=10)
axs[0].axhline(y=0, color='r', linestyle='--', linewidth=1)
axs[0].set_xlabel("Predicted values (ppm)")
axs[0].set_ylabel("Residuals (ppm)")
...
### Coverage Statistics {.unnumbered}

Coverage statistics assess whether the posterior predictive credible intervals contain the observed data at the expected frequency. For a well-calibrated model, approximately 50%, 80%, and 95% of observations should fall within the corresponding credible intervals.

```{python}
#| label: app-validation-coverage-co2

y_obs = df_collocated_co2["value_gb"].values
y_pred_samples = idata_co2.posterior_predictive["value_gb"].stack(sample=("chain", "draw")).values

# Calculate quantiles for each observation (percentiles along sample dimension)
n_obs = len(y_obs)
q25 = np.percentile(y_pred_samples, 25, axis=1)
q75 = np.percentile(y_pred_samples, 75, axis=1)
q10 = np.percentile(y_pred_samples, 10, axis=1)
q90 = np.percentile(y_pred_samples, 90, axis=1)
q025 = np.percentile(y_pred_samples, 2.5, axis=1)
q975 = np.percentile(y_pred_samples, 97.5, axis=1)

coverage_50 = np.sum((y_obs >= q25) & (y_obs <= q75)) / n_obs
coverage_80 = np.sum((y_obs >= q10) & (y_obs <= q90)) / n_obs
coverage_95 = np.sum((y_obs >= q025) & (y_obs <= q975)) / n_obs

print(f"Coverage at 50% credible interval: {coverage_50:.3f} (expected: 0.500)")
print(f"Coverage at 80% credible interval: {coverage_80:.3f} (expected: 0.800)")
print(f"Coverage at 95% credible interval: {coverage_95:.3f} (expected: 0.950)")
```

### Quantitative Metrics {.unnumbered}

Quantitative metrics provide summary statistics comparing posterior mean predictions to observed values, including Root Mean Squared Error (RMSE), Mean Absolute Error (MAE), and the coefficient of determination (R²).

```{python}
#| label: app-validation-metrics-co2
...
metrics_df = pd.DataFrame({
    "Metric": ["RMSE (ppm)", "MAE (ppm)", "R²"],
    "Value": [f"{rmse:.3f}", f"{mae:.3f}", f"{r2:.3f}"]
})
...
```{python}
#| label: app-validation-residuals-ch4

model_ch4_pred = model_ch4
model_ch4_pred.predict(idata_ch4, kind="response")

# Compute residuals using posterior mean predictions
y_obs = df_collocated_ch4["value_gb"].values
y_pred_mean = idata_ch4.posterior_predictive["value_gb"].mean(dim=("chain", "draw")).values
residuals = y_obs - y_pred_mean
...
axs[0].scatter(y_pred_mean, residuals, alpha=0.5, s=10)
axs[0].axhline(y=0, color='r', linestyle='--', linewidth=1)
axs[0].set_xlabel("Predicted values (ppb)")
axs[0].set_ylabel("Residuals (ppb)")
axs[0].set_title("Residuals vs. Predicted Values")
axs[0].spines[["right", "top"]].set_visible(False)
...
```{python}
#| label: app-validation-coverage-ch4

# Ensure posterior predictions are available
y_obs = df_collocated_ch4["value_gb"].values
y_pred_samples = idata_ch4.posterior_predictive["value_gb"].stack(sample=("chain", "draw")).values
...
```{python}
#| label: app-validation-metrics-ch4
