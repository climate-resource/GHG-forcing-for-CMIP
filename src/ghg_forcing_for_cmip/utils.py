...
def clean_and_save(
    df: pd.DataFrame, gas: str, save_to_path: str, 
    ...
    # save as csv
    df.to_csv(save_to_path + gas + f"/{gas}_{measurement_type}_raw.csv", index=False)
    ...
    ds = ds.assign_coords(
        {"year": year, "month": month, lat: latitude, lon: longitude}
    )
